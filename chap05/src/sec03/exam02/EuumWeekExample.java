...
public class EuumWeekExample {
	...
	public static void main(String[] args) {
		...
		// 열거 타입 변수에는 정의된 열거 상수 값만 저장 가능
			...
			// 열거 객체의 문자열이 출력(열거 상수의 이름과 똑같이 문자열을 갖는다.)
			...
			// 정리: 열거 타입의 이점
			// 1. 특정 범위의 값만 사용 가능하므로 타입 안정성 보장된다.(잘못된 값이 들어가면 바로 컴파일 에러 발생)
			// 2. 값들이 명시적으로 정의되어 있어 가독성이 올라간다.(코드를 읽을 때 쉽게 이해 가능)
			// 3. 값이 추가되거나 변경되는 경우 한 곳에서만 변경하면 되기 때문에 유지보수(관리)가 용이하다.
			// 4. 자동완성이 지원된다.
