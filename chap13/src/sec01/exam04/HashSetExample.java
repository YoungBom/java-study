...
//		Set<String> set = new HashSet<String>();
		...
		set.add("JDBC");
		set.add("Servlet/JSP");
		set.add("Java"); // 중복이라 객체가 4개만 표시된다.
		set.add("iBatis");
		...
		// 객체의 hashCode()와 equlas()를 기준으로 판단
		set.remove("JDBC"); // 1객의 객체삭제
		set.remove("iBatis");
		...
		// HashSet과 집합
		// 수학의 집합으로 효율적으로 처리하기에 적합한 구조
		// 두 집합(HashSet)의 합집함, 교집합, 차집합 연산 가능
...
//		Collections.sort(fruits); // 불가(List 계열만 가능)
		...
		// !!(중요) Set 활용 팁: ArrayList의 요소에서 중복 제거하기
		...
		// ArrayList로 만든것을 중복제거할때 HashSet을 이용해서 중복제거 작업을 한다
