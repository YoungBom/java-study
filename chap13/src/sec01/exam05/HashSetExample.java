...
public class HashSetExample {
	...
	public static void main(String[] args) {
		...
		set.add(new Member("김성대", 31));
		set.add(new Member("김성대", 31)); //

//		set.remove(new Member("김성대", 31));
		...
		// => Member 클래스가 Compareble 인터페이스를 구현

		Set<Member> members = new HashSet<Member>();
		members.add(new Member("디엔냥", 1731));
		members.add(new Member("봄동석", 1716));
		members.add(new Member("황돔", 1730));
		...
		// 다시 Set으로 변환(순서 유지하려면 LinkedHashSet으로 한다.)
		...
		List<Member> dedupList = new ArrayList<Member>(List);
			System.out.println(List);
			
			System.out.println("중복 제거든 리스트:");
