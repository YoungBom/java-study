...
public class FileExample {
	...
	public static void main(String[] args) {
		...
		// 파일 생성 >> (파일이 생성되있지 않다면(느낌표라 부정형))) 파일을 생성해라
		try {
			if (!file1.exists())
				file1.createNewFile();
			if (!file2.exists())
...
//		if (!dirTest.exists()) dirTest.mkdir(); //경로상 맨뒤 폴더만 생성(앞에 상위폴더가 있어야 함)
		if (!dirTest.exists())
		...
		// 파일 삭제
		if (file3.delete()) {
			System.out.println("test3.txt 파일 삭제 완료");
		...
		File parentDir = dir.getParentFile(); // Temp라는파일을 객체로 만들어서 parentDir의 변수값에 저장
...
//				System.out.println(file.lastModified()); // 확인용
				// (참고)
				// UNIX 시간(에포크 시간): 1970년 1월 1시 0시 0분 0초 UTC 기준의 경과 밀리초
